...
function FaqItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className={`faq-card border rounded-[24px] relative overflow-hidden backdrop-blur-xl transition-all duration-500 ${isOpen ? 'bg-black border-white/20 shadow-[0_10px_30px_rgba(0,0,0,0.5)]' : 'bg-white/5 border-white/10 shadow-[0_5px_20px_rgba(0,0,0,0.3)] hover:bg-white/[0.07]'}`}>
      
      <div className={`absolute -top-32 -left-32 w-64 h-64 rounded-full pointer-events-none transition-all duration-500 blur-[100px] ${isOpen ? 'bg-[#0175ff]/20 opacity-100' : 'bg-[#0175ff]/10 opacity-0'}`} />
      
      <button 
        onClick={() => setIsOpen(!isOpen)} 
        className="w-full text-left px-5 md:px-8 flex justify-between items-center gap-4 md:gap-6 group relative z-10 min-h-[96px] md:min-h-[104px]"
      >
        {/* Cambiado md:text-lg por md:text-[18px] para evitar bugs globales */}
        <span className="font-medium text-[17px] md:text-[18px] text-white leading-snug">
          {q}
        </span>
        
        <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 transition-all duration-500 border ${isOpen ? 'bg-white text-black border-white rotate-45 shadow-[0_0_15px_rgba(255,255,255,0.3)]' : 'bg-black/50 border-white/10 text-white/80 group-hover:bg-white/10'}`}>
      ...
      >
        {/* SOLUCIÓN: Cambiado md:text-base por md:text-[16px] para matar el bug de color negro */}
        <p className="px-5 pb-5 md:px-8 md:pb-6 text-white/80 text-[15px] md:text-[16px] leading-relaxed">
          {a}
        </p>
      </div>
    </div>
...
export function FAQ() {
  ...
  return (
    <section id="faq" ref={rootRef} className="relative w-full py-32 bg-[#020202] overflow-hidden font-sans border-t border-white/5">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10">
        ...
        <div className="faq-header text-center mb-20 flex flex-col items-center">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-white/10 bg-white/5 mb-6 shadow-[inset_0_1px_4px_rgba(255,255,255,0.1)]">
            <MessageSquare size={12} className="text-[#0175ff]" />
          ...
          <p className="text-[18px] text-white/50 max-w-xl mx-auto text-balance">
            Respuestas a las preguntas más comunes sobre nuestros servicios, procesos e integraciones empresariales.
          </p>
        </div>

        <div className="w-[85%] sm:w-[90%] md:w-full mx-auto grid lg:grid-cols-2 gap-4 md:gap-6 items-start">
