...
export function Hero() {
  ...
  useGSAP(
    () => {
      // 1. TIMELINE INICIAL (Animación de desenfoque al estilo exacto de la calculadora)
      const tl = gsap.timeline();

      // Animamos en cascada el badge, las líneas del título, el subtítulo y el botón
      ...
      // 2. TIMELINE INDEPENDIENTE PARA LAS MÉTRICAS
  ...
  return (
    <section ref={root} id="top" className="relative min-h-screen flex flex-col justify-start overflow-hidden pt-32 pb-24">
      ...
      <div className="hero-text-content relative z-10 w-full px-6 flex flex-col items-center text-center">
        {/* Badge */}
        <div className="hero-badge mb-6 inline-flex items-center gap-2 px-4 py-2 rounded-full border border-white/10 bg-white/5 backdrop-blur-md">
          <span className="text-[10px] sm:text-xs font-medium text-white/80 uppercase tracking-[0.05em]">
            Plataforma de digitalización
          </span>
        </div>

        {/* Titular (Sin overflow-hidden para que el Blur fluya libremente) */}
      ...
      {/* Dashboard Visual */}
      <div className="relative z-10 w-full max-w-5xl mx-auto mt-20 px-4 sm:px-6" style={{ perspective: '1200px' }}>
        <div 
          ref={dashboardRef} 
          className="relative rounded-2xl md:rounded-3xl overflow-hidden border border-white/10 shadow-[0_0_80px_rgba(255,255,255,0.03)] bg-black/40 backdrop-blur-xl"
          style={{
        ...
        >
          ...
          <img 
            src="https://live.staticflickr.com/65535/55519868727_2ec120cd70_b.jpg" 
      ...
      {/* Barra de Estadísticas estilo COSMOQ */}
      <div className="stats-container-wrapper relative z-10 w-full max-w-5xl mx-auto mt-20 px-4 sm:px-6">
        <div className="absolute inset-x-12 -top-4 h-20 bg-blue-500/5 blur-3xl rounded-full pointer-events-none" />
        ...
        <div className="stats-outer-border relative rounded-2xl md:rounded-3xl p-[1px] bg-gradient-to-b from-white/15 via-white/5 to-transparent shadow-[0_8px_32px_rgba(0,0,0,0.3)]">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-px rounded-[23px] overflow-hidden bg-white/5 backdrop-blur-xl">
            {[
              ...
              <div
                key={i}
                className="hero-stat-card group relative p-6 sm:p-8 text-center bg-black/50 flex flex-col justify-center items-center overflow-hidden"
              >
                ...
                <div className="overflow-hidden mb-1.5 w-full">
                  <div className="hero-stat-value font-display text-3xl sm:text-4xl md:text-5xl font-semibold text-white tracking-[-0.03em]">
                    {stat.value}
                  </div>
                </div>

                <div className="hero-stat-label w-full">
                  <div className="text-[11px] sm:text-xs font-medium text-white/80 uppercase tracking-[0.08em] mt-1 group-hover:text-white transition-colors">
                    {stat.label}
                  </div>
                  <div className="text-[9px] sm:text-[10px] text-white/40 tracking-wide mt-1.5 font-light group-hover:text-white/60 transition-colors">
                    {stat.sub}
                  </div>
                </div>
              </div>
